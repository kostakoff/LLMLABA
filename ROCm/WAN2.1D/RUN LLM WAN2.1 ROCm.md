# RUN LLM WAN 2.1 1.3b from scratch ROCm

## Requirments 
- AMD Mi200 64Gb VRAM
- Workstation 64 GB RAM, 200GB SSD, 750W Power supply 
...
```python
# pip install diffusers>=0.34 transformers ftfy accelerate
# см. инструкции PyTorch ROCm для вашей версии ROCm
...
# 1.3B есть в diffusers-формате
model_id = "/home/sysadmin/llm/wan2.1d"  # при желании можно: "Wan-AI/Wan2.1-VACE-1.3B"

# 2) грузим VAE отдельно во float32 и сам пайплайн в bf16
vae = AutoModel.from_pretrained(model_id, subfolder="vae", torch_dtype=torch.float32)
pipe = WanVACEPipeline.from_pretrained(model_id, vae=vae, torch_dtype=torch.bfloat16)

# 3) ROCm: устройство по-прежнему "cuda" (это корректно для ROCm-сборки PyTorch)
pipe.to("cuda")
